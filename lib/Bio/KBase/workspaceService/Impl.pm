...
sub _initializeWorkspace {
	...
	if (defined($wsmeta)) {
		...
		my $cursor = $mongodb->workspaceDataObjects->find({});
		my $objHash = {};
		while (my $object = $cursor->next) {
			my $newObject = Bio::KBase::workspaceService::DataObject->new({
				parent => $self,
...
sub object_to_html
{
    ...
    $html = $self->_tohtml($obj);
    $self->_clearContext();
    #END object_to_html
    my @_bad_returns;
    (!ref($html)) or push(@_bad_returns, "Invalid type for return variable \"html\" (value was \"$html\")");
    if (@_bad_returns) {
...
sub objectref_to_html
{
    ...
    $html = $self->_tohtml($obj);
    $self->_clearContext();
    #END objectref_to_html
    my @_bad_returns;
    (!ref($html)) or push(@_bad_returns, "Invalid type for return variable \"html\" (value was \"$html\")");
    if (@_bad_returns) {
	my $msg = "Invalid returns passed to objectref_to_html:\n" . join("", map { "\t$_\n" } @_bad_returns);
	Bio::KBase::Exceptions::ArgumentValidationError->throw(error => $msg,
							       method_name => 'objectref_to_html');
    }
    return($html);
...
sub queue_job
{
    ...
    my $cursor = $self->_mongodb()->jobObjects->find({id => $params->{jobid}});
    if (my $object = $cursor->next) {
